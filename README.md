...
Question 1 (Project & Application Configuration):
  ...
  To combat this, all of the shared data is now stored in a seperate class called "DataStore" rather than using
  static fields. Since static fields belong to the class instead of an instance, they can remain for indefinetely
...
Question 2 (The ”Discovery” Endpoint):
  HATEOAS stands for Hypermedia As The Engine Of Application State. Essentially, the API responses includes 
  links to other parts of the API that relate to it, instead of forcing the client to know the URL's already. 
  As an example, "GET/api/v1", by looking at this, we can show that the project links to "api/v1/Rooms" and also
  "api/v1/Sensors". Meaning that a client can simply start at the root endpoint and find all other details from it. 
  This is more suitable than static documenting as clients wouldn't have to hardcode URLs, 
...
Question 1 (Room Resource Implementation):
  ...
  amount of  rooms. However, the client would need to make seperate requests for each individual room 
  ...
  would recieve all the needed information in a singular request. 
  For a majority of cases, this seems to be the more suitable approach, 
...
Question 2 (Room Deletion & Safety Logic):
  ...
  rooms state essentially remains unchanged as it no longer exists. The only thing that changes is the status code, 
...
Question 1 (Sensor Resource & Integrity): 
  ...
  is a client sends data in an alternate format like "xml" or "plaintext", the request is automatically rejected. 
	Jersey would then return an error code of 415, meaning "Unsupported Media Type", as the resource method won't run.
	There isn't necessarily a need for additional validation code. 

Question 2 (Filtered Retrieval & Search): 
  By using "?type = CO2" aas a query parameter instead of having the filter in the path akin to "/Sensors/Type/CO2", 
  there are advantages such as: the path of "/api/v1/Sensors" now always refers to the sensors collection, mutiple
  filters can also be added rather simply for example, "?type = CO2 & status = ACTIVE", this also doesn't change
  the URL structure.

Part 4: Deep Nesting with Sub - Resources
Question 1 (The Sub-Resource Locator Pattern):
  The Sub-Resource Locator Pattern pattern allows for a parent resource to delegate handling nested paths to child 
  ...
  that a reading belongs to a sensor, so it is more befitting that data is handled within it's specific context.

Part 5: Advanced Error Handling, Exception Mapping & Logging
Question 1 (Dependency Validation):
  HTTP 404 means that a requested URL was not found, meaning that if a clinet gives a sensor with an invalid roomId, 
  the URL would still be fine since the problem is'nt the URL itself but the data. 
  ...
  This therefore gives the client a more clear idea of the potential error. 
...
Question 2 (The Global Safety Net):
  By having stack traces exposed to external users, several cybersecurity issues can occur, such as:
    - having the class and package names be revealed, essentially providing a map of the code structure
